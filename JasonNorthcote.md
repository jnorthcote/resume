...
Transitioned to an expanded role as lead developer on all aspects related to Blue Ocean automation as part of a `1-click` service registration, provisioning and deployment orchestration fabric
...
Lead development on containerization of the QBO application and conversion of the existing CICD process to a container native one from the initial proof-of-concept to production readiness
  ...
  * Created a parallel container centric CICD pipeline for Quickbooks
    ...
    * Converted the QBO shell execution plan into a scripted pipeline with the initial goal being functional parity followed by enhancements to reduce productivity losses caused by broken builds due to defect promotion.
